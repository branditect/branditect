...
export default function BrandStrategyPage() {
    ...
    try {
      ...
      try {
        ...
        if (saved && saved.length > 0) {
          ...
          if (completeness(readStrategy(strategyJson)).filled > 0) {
            setScreen("view");
          } else {
            setScreen("output");
          }
  ...
  // One shape, from lib/strategy.ts. Legacy records are migrated on read.
  const [strategyDoc, setStrategyDoc] = useState<BrandStrategy>(EMPTY_STRATEGY);
  // Per-section edit forms are not built yet. Until they are, Edit opens the
  // questionnaire rather than being a control that silently does nothing.
  const openQuestionnaire = () => setScreen("questions");
