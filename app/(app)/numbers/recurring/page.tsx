"use client";

import { useState } from "react";
import { formatMoney, DEFAULT_CURRENCY } from "@/lib/products";
import { recurring } from "@/lib/numbers";
import { CalcShell, Field, Panel, Readout, toNum } from "@/components/numbers/calc-shell";
import { useT } from "@/lib/i18n/use-t.tsx";

export default function RecurringCalculator() {
  const t = useT();
  const [arpu, setArpu] = useState("");
  const [churn, setChurn] = useState("");
  const [cac, setCac] = useState("");
  const [gm, setGm] = useState("80");
  const currency = DEFAULT_CURRENCY;

  const a = toNum(arpu);
  const ch = toNum(churn);
  const cc = toNum(cac);
  const g = toNum(gm);
  const r = a != null && ch != null && cc != null && g != null
    ? recurring({ arpu: a, churnPct: ch, cac: cc, grossMarginPct: g })
    : null;

  const unhealthy = r != null && r.ltvToCac < 1;

  return (
    <CalcShell n={4} tone="blue" title={t("numbers.recurring")}
      promise={t("num.recurringLede")}>
      <div className="grid grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] items-start gap-3 stack:grid-cols-1">
        <Panel title={t("numbers.subscriptionNumbers")}>
          <p className="mt-1 text-xs font-medium leading-[1.55] text-muted">
            {t("num.rec.lede")}
          </p>
          <div className="mt-4 flex flex-col gap-2.5">
            <Field label={t("num.rec.revenuePerCustomer")} hint={t("num.rec.perMonthNet")} value={arpu} onChange={setArpu} suffix={currency} />
            <Field label={t("num.rec.grossMargin")} hint={t("num.rec.afterCostToServe")} value={gm} onChange={setGm} suffix="%" />
            <Field label={t("num.rec.monthlyChurn")} hint={t("num.rec.shareWhoLeave")} value={churn} onChange={setChurn} suffix="%" />
            <Field label={t("num.costToAcquire")} hint={t("num.rec.acquisitionHint")} value={cac} onChange={setCac} suffix={currency} />
          </div>
        </Panel>

        <div className="flex flex-col gap-3">
          <Readout tone="blue" label={t("num.rec.ltv")} warn={unhealthy}
            value={r == null ? "—" : formatMoney(r.ltv, currency)}
            sub={r == null
              ? ch === 0
                ? "At zero churn nobody ever leaves, so lifetime is infinite — not a number worth showing."
                : t("num.rec.fillFour")
              : <>{r.lifetimeMonths.toFixed(1)} months at {formatMoney((a ?? 0) * ((g ?? 0) / 100), currency)} gross profit a month. Computed on gross profit, not revenue — revenue ignores what serving them costs.</>}
          />
          <Readout tone="blue" label={t("num.rec.payback")}
            value={r == null ? "—" : r.paybackMonths === Infinity ? t("common.never") : `${r.paybackMonths.toFixed(1)} months`}
            sub={r == null ? undefined
              : r.paybackMonths === Infinity
                ? t("num.rec.noMargin")
                : t("num.rec.paybackNote", { ltvToCac: r.ltvToCac.toFixed(1) })}
          />
          {unhealthy && (
            <p className="rounded-card border border-accent-line bg-tint-1 px-3.5 py-3 text-2xs font-medium leading-[1.6] text-accent-dark">
              {t("num.rec.worthLess")}
            </p>
          )}
        </div>
      </div>
    </CalcShell>
  );
}
