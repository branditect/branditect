...
The account row at the foot of the sidebar — avatar, name, org — is a **button**, not a static block. It opens a menu upward, anchored to the row.

```ts
interface AccountMenuProps {
  user: { name: string; org: string; initials: string; avatarUrl?: string };
...
| Item | State |
|---|---|
| Profile | `Soon` — renders, does nothing |
| Settings | `Soon` — renders, does nothing |
| Help | `Soon` — renders, does nothing |
| *separator* | |
| **Log out** | **Wired.** The only working item, and the only one in red. |

The three unwired items carry a small uppercase `Soon` tag and sit at `--muted-2` with `--faint` icons, so the menu tells the truth about itself before anyone clicks. **Do not use the `disabled` attribute** — a disabled item takes no focus and a keyboard user never learns it exists. Render them enabled with a no-op handler.

Log out goes last, after a separator, in `#c8402a`. It is the destructive one and it must not sit adjacent to anything a person clicks casually.

### Log out — the wired path

```ts
await supabase.auth.signOut();
router.replace("/login");   // replace, not push — back must not re-enter the app
```

`replace` matters. With `push`, the browser back button returns to a rendered app shell with stale data on screen until the auth guard catches up.

### Behaviour

- Toggles on click. `aria-haspopup="menu"`, `aria-expanded`, `aria-controls` on the trigger; `role="menu"` and `role="menuitem"` inside.
- Opening moves focus to the first item.
- Click anywhere outside closes it. **Escape** closes it and returns focus to the trigger.
- Up/down arrows walk the items and wrap.
- The chevron rotates 180° while open, and the row keeps `--tile` background so it reads as the active surface.

### Placement

Absolutely positioned `bottom: calc(100% + 8px)`, stretched to the row's width, inside a `position: relative` wrapper around the row. Anchoring to the row rather than the panel is what keeps it correct when the sidebar collapses on narrow screens.

Radius `14px`, `5px` padding, panel border `--rule`, and the deep shadow — this floats above content, so it needs more elevation than a card.
